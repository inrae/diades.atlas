...
dossier `inst/` et des tables contenues dans la base de données PostGre. 
...
+ `translation_iucn.csv` : contient les traductions des status IUCN. 

+ `inst/translation.csv` : contient tous les autres éléments traduisibles de l'interface utilisateur.

Ces fichiers partache la même structure. Ces derniers doivent posséder à minima: 
...
Elle comporte une description plus détaillée de le ligne en question.

Vous pouvez vérifier l'intégrité du csv à l'aide de la fonction `check_translation_csv()`:

```{r}
translation_help <- check_translation_csv(here("inst/translation_help.csv"))
head(translation_help)
```

```{r}
translation_iucn <- check_translation_csv(here("inst/translation_iucn.csv"))
head(translation_iucn)
```

```{r}
translation <- check_translation_csv(here("inst/translation.csv"))
head(translation)
```
...
Dans les fichiers manuellement mises à jour:
