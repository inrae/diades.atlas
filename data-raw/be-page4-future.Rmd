...
anthropogenic_mortality <- expand_anthropogenic_mortality(
  data_simulation[["data_hsi_nmax"]], mortalities)
...
results$other$survivingProportion['60', ]
results$other$eh2['60', ]
results$model$cnrmcm5$spawnersTo['60', ]
Nit_list$cnrmcm5['60', ]

basin <- data_simulation[["data_catchment"]] %>% 
  filter(basin_name == 'Garonne') %>%  
  collect() %>% 
  pull(basin_id)

# Plot Nit predictions
# debugonce('nit_feature_species')
